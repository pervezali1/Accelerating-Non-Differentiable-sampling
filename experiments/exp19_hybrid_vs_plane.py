...
# ------------------------------------------------- J = 0, the plane, the hybrid
def hybrid(a, b):
    """[[0, b, a], [-b, 0, b], [-a, -b, 0]]: the stiff-soft plane plus b x tridiagonal."""
    J = np.zeros((3, 3))
    J[0, 1], J[1, 0] = b, -b
    J[0, 2], J[2, 0] = a, -a
    J[1, 2], J[2, 1] = b, -b
...
A_HY, B_HY = 1.75, 0.1
FIELDS = [("zero",   r"$J = 0$",                                   None,            ETA["zero"]),
          ("plane",  rf"stiff$\leftrightarrow$soft,  $a = {A_HY:g}$,  $b = 0$",
                                                                   hybrid(A_HY, 0), 5.855338e-04),
          ("hybrid", rf"hybrid,  $a = {A_HY:g}$,  $b = {B_HY:g}$",  hybrid(A_HY, B_HY), 5.829086e-04)]
for k, _, J, eta in FIELDS:
    assert J is None or np.allclose(J, -J.T), k
    print(f"   {k:>6}  norm {0.0 if J is None else np.linalg.norm(J, 2):4.2f}   eta {eta:.6e}")
print("\nJ_a hybrid:\n", hybrid(A_HY, B_HY), flush=True)
...
def curve(J, eta, prior):
    ...
    for r in range(REPS):
        ...
        out.append(w)
    return list(map(float, np.mean(out, axis=0)))

curves, hits = {}, {}
for prior in PRIORS:
    curves[prior] = {k: curve(J, eta, prior) for k, _, J, eta in FIELDS}
    hits[prior] = {k: next((t for t, v in zip(record_at, curves[prior][k]) if v <= 2 * floor), -1)
                   for k, *_ in FIELDS}
    z = hits[prior]["zero"]
    print(f"{prior}: " + ",  ".join(
        f"{k} {hits[prior][k]}" + (f" ({z/hits[prior][k]:.2f}x)" if k != "zero" else "")
        for k, *_ in FIELDS), flush=True)

json.dump(dict(lam=LAM, a=A_HY, b=B_HY, floor=float(floor), rec=list(record_at),
               eta={k: e for k, _, _, e in FIELDS}, curves=curves, hits=hits),
          open("hybrid_vs_plane.json", "w"), indent=1)
...
for ax, prior in zip(axes, PRIORS):
    z = hits[prior]["zero"]
    for k, lab, J, eta in FIELDS:
        h = hits[prior][k]
        tag = f"   $\\bf{{{h}}}$ it, {z/h:.2f}$\\times$" if h > 0 else ""
        ax.plot(it, curves[prior][k], lw=2.2, ls="--" if k == "hybrid" else "-", label=lab + tag)
...
fig.tight_layout()
fig.savefig("mcp_hybrid_vs_plane.png", dpi=200)
print("wrote mcp_hybrid_vs_plane.png")
